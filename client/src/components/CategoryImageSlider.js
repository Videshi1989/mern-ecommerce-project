...

                {
                    records.length > 0 ?
						records.map((data,index) => 
                            ...
                            <SwiperSlide key={index}><Link  to={`/allproductview2/${data.categoryid}`}>{data.imagename ? <img loading='lazy'  alt="img" src={`${process.env.REACT_APP_BACK_END_URL}/uploads/${data.imagename}`} className='topimg' /> : <img src={`${process.env.REACT_APP_BACK_END_URL}/uploads/noimg.svg`} className='topimg' alt="img" />}</Link></SwiperSlide>
