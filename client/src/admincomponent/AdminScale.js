...
const AdminScale = () => {
	...
	const Pageheading = 'Scale';
	const Pagesubheading = 'Home / Scale';
	...
	{ 
		try 
		{
			const response = await fetch(`${ADMINAPI}/getallscale`,{
				method:"GET",
				headers:{
	...
	async function ShowpopupView(getid)
	{
	  ...
	  try{
		const response = await fetch(`${ADMINAPI}/getsinglescale/${getid}`,{
			method : 'GET',
			headers:{
						'Content-Type': 'application/json',
						Authorization : authorizationToken 
		...
		const singlescaledata = await response.json();
		if(singlescaledata)
		{
			Setviewdata(singlescaledata);
		}
		else
		{
			Setviewdata([]);
...
function downloadCSV(args) {  
		...
		if(records.length !== undefined && records.length !== null && records.length !== '') 
		{
			...
			filename = args.filename || 'Scale-List.csv';
    ...
    const columns=[
	   ...
	   const columnspdf = [
		{ header: "SCALE", dataKey: "scalename" },
				...
				if(event.target.value !== '')
					{
						const newData = allrecords.filter(row=>{
							return(
								   row.scalename.toLowerCase().includes(event.target.value.toLowerCase()) ||
    ...
    const handleSubmit = async (e) =>{      
		...
		if(scale.scalename === "" || scale.scalename === null || scale.scalename === undefined)
		{
			toast.error("Please enter scale name !");
			return;
		}
		else if(scale.status === "" || scale.status === null || scale.status === undefined)
		{
		...
		else
		{
			...
			const scalewithimgstatus = {...scale,...imgfeedback}
				
			try
			{
			  const response = await fetch(`${ADMINAPI}/createscale`,{
				  method:"POST",
				  headers:{
					  'Content-Type':'application/json',
					  Authorization : authorizationToken
				  },
				  body:JSON.stringify(trimSpaces(scalewithimgstatus))
   	...
   	const[edituserdata,Setedituserdata] = useState({scalename:"", status:""});
    ...
    const handleSubmitEdit = async (e) =>{      
		...
		if(edituserdata.scalename === "" || edituserdata.scalename === null || edituserdata.scalename === undefined)
		{
			toast.error("Please enter scale name !");
			return;
		}
		...
		else
		{
			...
			const imgfeedback = await imgresponse.json();
			const brandwithimgstatus = {...edituserdata,...imgfeedback}
			
			try
			{
			  const response = await fetch(`${ADMINAPI}/geteditscale/${editid}`,{
				  method:"PATCH",
				  headers:{
					  'Content-Type': 'application/json',
					  Authorization : authorizationToken 
				  ...
				  body:JSON.stringify(trimSpaces(brandwithimgstatus))
			  });
...
async function ShowpopupEdit(getid)
{
	...
	if(singlescaledata)
	{
		Setedituserdata(singlescaledata);
	}
	else
	{
		Setedituserdata({brandname:"", status:""});
	}
   ...
   catch(error)
   {
	...
	const DeleteRecord = async(e)=>{
		...
		try
		{
			const response = await fetch(`${ADMINAPI}/deletescale/${deleteid}`,{
				method : 'DELETE',
				headers:{
					'Content-Type': 'application/json',
					Authorization : authorizationToken 
	...
	const FilterByDate = async ()=>{
		...
		else
		{
			...
			try
			{
				const response = await fetch(`${ADMINAPI}/filterscalebydate`,{
					method:"POST",
					headers:{
						'Content-Type': 'application/json',
						Authorization : authorizationToken 
	 ...
	 <div className="viewheight viewheightmobile" id="style-scroll2">
		<div className="row mt-3">
			<div className="col-2 mobwidth100admin">
				<span className="cont-heading">Scale:</span>
			</div> 
			<div className="col-10 mobwidth100admin">
				<span className="cont-cont">{viewdata.scalename}</span>
			</div>
  ...
  <div className="popupaddnew animate-zoom">
	...
	<div className="container-fluid padd_0" >
	 <div className="viewheight" id="style-scroll2">
		<div className="row mt-3">
			<div className="col-2 mobwidth100admin">
				<span className="cont-heading">Scale Name:</span>
			</div>
			<div className="col-10 mobwidth100admin">
				<input name="scalename" id="scalename" onChange={handleInput} type="text" className="admin-control" value={scale.scalename} placeholder="Enter scale name" />
			</div>
		</div>
		<div className="row mt-3">
			<div className="col-2 mobwidth100admin">
			...
			<div className="col-10 mobwidth100admin">
				<input name="scaleimg" id="scaleimg" onChange={(e)=>{ setImg(e.target.files[0]); setpreviewimg(URL.createObjectURL(e.target.files[0])); }} type="file" accept="image/png,image/jpg,image/jpeg,image/svg+xml,image/gif"  className="admin-control"  />
	...
	<div className="container-fluid padd_0" >
	 <div className="viewheight" id="style-scroll2">
		<div className="row mt-3">
			...
			<div className="col-10 mobwidth100admin">
				<input name="scalename" id="scalename" value={edituserdata.scalename} onChange={handleInputEdit} type="text" className="admin-control"  placeholder="Enter scale name" />
			</div>
		</div>
		<div className="row mt-3">
			<div className="col-2 mobwidth100admin">
			...
			<div className="col-10 mobwidth100admin">
				<input name="scaleimg" id="scaleimg" onChange={(e)=>{ setdisplaypreviewimg(false); setImg(e.target.files[0]); setpreviewimg(URL.createObjectURL(e.target.files[0])); }} type="file" accept="image/png,image/jpg,image/jpeg,image/svg+xml,image/gif" className="admin-control"  />
				<span className='imginst'>upload only png,jpg,jpeg,svg and gif file.</span>
