...
const AdminBrand = () => {
	...
	const Pageheading = 'Brand';
	const Pagesubheading = 'Home / Brand';
	...
	{ 
		try 
		{
			const response = await fetch(`${ADMINAPI}/getallbrand`,{
				method:"GET",
				headers:{
	...
	async function ShowpopupView(getid)
	{
	  ...
	  try{
		const response = await fetch(`${ADMINAPI}/getsinglebrand/${getid}`,{
			method : 'GET',
			headers:{
						'Content-Type': 'application/json',
						Authorization : authorizationToken 
		...
		const singlebranddata = await response.json();
		//console.log("welllllcome",singlebranddata)
		if(singlebranddata)
		{
			Setviewdata(singlebranddata);
		}
		else
		{
			Setviewdata([]);
...
function downloadCSV(args) {  
		...
		if(records.length !== undefined && records.length !== null && records.length !== '') 
		{
			...
			filename = args.filename || 'Brand-List.csv';
				...
				if(event.target.value !== '')
					{
						const newData = allrecords.filter(row=>{
							return(
								   row.brandname.toLowerCase().includes(event.target.value.toLowerCase()) ||
    ...
    const handleSubmit = async (e) =>{      
		...
		if(brand.brandname === "" || brand.brandname === null || brand.brandname === undefined)
		{
			toast.error("Please enter brand name !");
			return;
		}
		else if(brand.status === "" || brand.status === null || brand.status === undefined)
		{
		...
		else
		{
			...
			const brandwithimgstatus = {...brand,...imgfeedback}
				
			try
			{
			  const response = await fetch(`${ADMINAPI}/createbrand`,{
				  method:"POST",
				  headers:{
					  'Content-Type':'application/json',
					  Authorization : authorizationToken
   	...
   	const[edituserdata,Setedituserdata] = useState({brandname:"", status:""});
    ...
    const handleSubmitEdit = async (e) =>{      
		...
		if(edituserdata.brandname === "" || edituserdata.brandname === null || edituserdata.brandname === undefined)
		{
			toast.error("Please enter brand name !");
			return;
		}
		...
		else
		{
			...
			try
			{
			  const response = await fetch(`${ADMINAPI}/geteditbrand/${editid}`,{
				  method:"PATCH",
				  headers:{
					  'Content-Type': 'application/json',
					  Authorization : authorizationToken 
...
async function ShowpopupEdit(getid)
{
	...
	if(singlebranddata)
	{
		Setedituserdata(singlebranddata);
	}
	else
	{
   ...
   catch(error)
   {
	...
	const DeleteRecord = async(e)=>{
		...
		try
		{
			const response = await fetch(`${ADMINAPI}/deletebrand/${deleteid}`,{
				method : 'DELETE',
				headers:{
					'Content-Type': 'application/json',
					Authorization : authorizationToken 
	...
	const FilterByDate = async ()=>{
		...
		else
		{
			...
			try
			{
				const response = await fetch(`${ADMINAPI}/filterbrandbydate`,{
					method:"POST",
					headers:{
						'Content-Type': 'application/json',
						Authorization : authorizationToken 
	 ...
	 <div className="viewheight viewheightmobile" id="style-scroll2">
		<div className="row mt-3">
			...
			<div className="col-10 mobwidth100admin">
				<span className="cont-cont">{viewdata.brandname}</span>
			</div>
  ...
  <div className="popupaddnew animate-zoom">
	...
	<div className="container-fluid padd_0" >
	 <div className="viewheight" id="style-scroll2">
		<div className="row mt-3">
			<div className="col-2 mobwidth100admin">
				<span className="cont-heading">Brand Name:</span>
			</div>
			<div className="col-10 mobwidth100admin">
				<input name="brandname" id="brandname" onChange={handleInput} type="text" className="admin-control" value={brand.brandname} placeholder="Enter brand name" />
			</div>
		</div>
		<div className="row mt-3">
			<div className="col-2 mobwidth100admin">
	...
	<div className="container-fluid padd_0" >
	 <div className="viewheight" id="style-scroll2">
		<div className="row mt-3">
			...
			<div className="col-10 mobwidth100admin">
				<input name="brandname" id="brandname" value={edituserdata.brandname} onChange={handleInputEdit} type="text" className="admin-control"  placeholder="Enter brand name" />
			</div>
		</div>
		<div className="row mt-3">
			<div className="col-2 mobwidth100admin">
			...
			<div className="col-10 mobwidth100admin">
				<input name="brandimg" id="brandimg" onChange={(e)=>{ setdisplaypreviewimg(false); setImg(e.target.files[0]); setpreviewimg(URL.createObjectURL(e.target.files[0])); }} type="file" accept="image/png,image/jpg,image/jpeg,image/svg+xml,image/gif" className="admin-control"  />
				<span className='imginst'>upload only png,jpg,jpeg,svg and gif file.</span>
