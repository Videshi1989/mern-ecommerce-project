...
function downloadCSV(args) {  
    ...
    const handleInput = (e)=>{        //calling on typing
	 ...
	 const handleSubmit = async (e) =>{      //calling on submit
		 ...
		 if(create.fullname === "" || create.fullname === null || create.fullname === undefined)
			{
				toast.error("Please enter full name !");
				return;
			}
			else if(create.email === "" || create.email === null || create.email === undefined)
			{
				toast.error("Please enter email !");
				return;
			}
			else if(!validator.validate(create.email))
			{
				toast.error("Please enter valid email !");
				return;
			}
			else if(create.mobileno === '' || create.mobileno === null || create.mobileno === undefined)
			{
				toast.error("Please enter mobile number !");
				return;
			}
			else if(!Number(create.mobileno))
			{
				toast.error("Mobile number must be digits only !");
				return;
			}
			else if(create.mobileno.toString().length !== 10 )
			{
				toast.error("Please enter 10 digit mobile number !");
				return;
			}
			else if(create.password === "" || create.password === null || create.password === undefined)
			{
				toast.error("Please enter password !");
				return;
			}
			else if(create.cnfpassword === "" || create.cnfpassword === null || create.cnfpassword === undefined)
			{
				toast.error("Please enter confirm password !");
				return;
			}
			else if(create.password != create.cnfpassword)
			{
				toast.error("Password and confirm password did not match !" );
				return;
			}
			else if(create.usertype === '' || create.usertype === null || create.usertype === undefined)
			{
				toast.error("Please select user type !");
				return;
			}
			else if(create.status === '' || create.status === null || create.status === undefined)
			{
