...
const AdminSubCategoryImageSlider = () => {
	const[showloader,Setshowloader] = useState(true);     //loading effect
	const Pageheading = 'Subcategory Slider Section';
	const Pagesubheading = 'Home / Subcategory Slider Section';
 ...
 //const[chk,setchk]=useState([])
	...
	const getAllRecords = async ()=>
	{
		try 
		{
			const response = await fetch(`${ADMINAPI}/getallsubcategoryimageslider`,{
				method:"GET",
				headers:{
			 ...
			 else
			 {
				...
				if(data.length > 0 && data2.length > 0 && data3.length > 0 )
				{
					...
					data.forEach(async (season, index) => {
				 	seasonsList.push({categoryname:data2.find((result)=> result._id===season.categoryid).categoryname, subcategoryname:data3.find((result)=> result._id===season.subcategoryid).subcategoryname, created_date:season.created_date ,status:season.status,_id:season._id});
					});
	...
	async function ShowpopupView(getid)
	{
	  ...
	  try{
		const response = await fetch(`${ADMINAPI}/getsinglesubcategoryimageslider/${getid}`,{
			method : 'GET',
			headers:{
							'Content-Type': 'application/json',
							Authorization : authorizationToken 
...
async function ShowpopupEdit(getid)
{
  ...
  document.getElementById('popupedit').style='display:block';
	...
	{
		Setedituserdata({categoryid:"",subcategoryid:"",status:""});
	}
...
function downloadCSV(args) {  
		...
		if(records.length !== undefined && records.length !== null && records.length !== '') 
		{
			...
			filename = args.filename || 'Subcategory-Slider.csv';
    ...
    const handleSubmit = async (e) =>{      //calling on submit
		...
		if(childcategory.categoryid === "" || childcategory.categoryid === null || childcategory.categoryid === undefined)
		{
			toast.error("Please select category !");
			return;
		}
		else if(childcategory.subcategoryid === "" || childcategory.subcategoryid === null || childcategory.subcategoryid === undefined)
		{
			toast.error("Please select subcategory !");
			return;
		}
		else if(childcategory.status === "" || childcategory.status === null || childcategory.status === undefined)
		{
		...
		else
		{
			...
			try
			{
			  const response = await fetch(`${ADMINAPI}/createsubcategoryimageslider`,{
				  method:"POST",
				  headers:{
								'Content-Type': 'application/json',
								Authorization : authorizationToken 
	...
	//////for Edit Record
   	const[edituserdata,Setedituserdata] = useState({categoryid:"",subcategoryid:"",status:""});
    ...
    const handleSubmitEdit = async (e) =>{      //calling on submit
		...
		else
		{
			...
			try
			{
			  const response = await fetch(`${ADMINAPI}/geteditsubcategoryimageslider/${editid}`,{
				  method:"PATCH",
				  headers:{
					  'Content-Type': 'application/json',
					  Authorization : authorizationToken 
	...
	const DeleteRecord = async(e)=>{
		...
		try
		{
			const response = await fetch(`${ADMINAPI}/deletesubcategoryimageslider/${deleteid}`,{
				method : 'DELETE',
				headers:{
							'Content-Type': 'application/json',
							Authorization : authorizationToken 
	...
	const FilterByDate = async ()=>{
		...
		else
		{
			...
			try
			{
				const response = await fetch(`${ADMINAPI}/filtersubcategoryimagesliderbydate`,{
					method:"POST",
					headers:{
								'Content-Type': 'application/json',
								Authorization : authorizationToken 
	...
	<div className="container-fluid padd_0" >
	 ...
	 <div className="row mt-3">
		...
		<div className="row mt-3">
			...
			<div className="col-10 mobwidth100admin">
				 <select id="subcategoryid" name="subcategoryid" value={edituserdata.subcategoryid} className="admin-control" onChange={handleInputEdit}>
					<option value="">Select sub category</option>
					{
...
export default AdminSubCategoryImageSlider